...
var UserLibrary = function() {
    return {
        ...
        queryMatchingAlgorithm: function(options, callback){
            ...
            // }
            if(options.industry){
                 innerQuery.$or.push({ industry: options.industry});
            }
            if(options.city){
                 innerQuery.$or.push({ city: options.city});
            }

            if(options.fullName){                 
                 innerQuery.$or.push({ fullName: { $regex : new RegExp(options.fullName, "i") } });                 
            }

            if(options.college){
                 innerQuery.$or.push({ college: options.college});
            }

            if(options.gender){
                innerQuery.$or.push({ gender: { $regex : new RegExp(options.gender, "i") } });                 
                 // innerQuery.$or.push({ gender: options.gender});
            }
